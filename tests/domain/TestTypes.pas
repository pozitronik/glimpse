...
type
  ...
  TTestTypes = class
  public
    { Enum ordering is load-bearing: used as array indices in ToolbarLayout }
...
begin
  {The settings dialog casts the combo box index straight to
   TProgressBarLayout (SettingsDlg.ControlsToSettings) and uses the
   enum's Ord value to set the index back (SettingsDlg.SettingsToControls).
