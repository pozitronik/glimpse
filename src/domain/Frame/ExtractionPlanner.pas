{Pure worker chunk planning and settings change detection.}
unit ExtractionPlanner;
...
{MaxWorkers=0 means one worker per frame. MaxThreads caps the actual
 thread count (-1 = no limit, 0 = CPU core count). Result is never
 empty when FrameCount > 0.}
...
{Returns 0 when scaling is not needed (video already fits or feature off).
 AAspectRatio is height/width.}
...
{Returns 0 (no scaling) when AScaledExtraction is off, or when the
 source's bigger side already fits within AMaxFrameSide. Decoupled from
 CalcExtractionMaxSide because saving is not viewport-driven.}
...
begin
  ...
  {Round to bucket for cache key stability}
...
begin
  ...
  {Slider movement only triggers re-extract when random is enabled.}
