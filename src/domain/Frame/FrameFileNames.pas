...
begin
  ...
  Result := Format('%s_frame_%.2d_%s%s', [BaseName, AFrameIndex + 1, FormatTimecodeForFilename(ATimeOffset), SaveFormatExtension(AFormat)]);
end;

function GenerateCombinedFileName(const AVideoFileName: string; AFormat: TSaveFormat): string;
begin
  Result := ChangeFileExt(ExtractFileName(AVideoFileName), '') + '_combined' + SaveFormatExtension(AFormat);
end;
